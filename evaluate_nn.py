...
if config['tagger'] == 'LSTM':
    ...
    x_embeddings, x, y, tags, voc, voc_inv, tag_dict_inv, sent_lens = load_processed_data(config_lstm['processed_data_location'])
    x_dev, tags_dev, sent_lens_dev = load_processed_data(config_lstm['processed_dev_data_location'], True)
    logging.info('Data loaded!')
    tagger = BiLSTM(config, tag_dict_inv, x.shape[1], x_embeddings, len(tag_dict_inv))
